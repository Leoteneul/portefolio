...
function setPos(x, y){
  ...
  const radius = 13
  const posVector = new THREE.Vector3().setFromSpherical(spherical.set(radius, x ,  y ))
  return posVector
}
function setTabWord(){
...
function Word({ children, ...props }) {
  ...
  const fontProps = { font: '/AlfaSlab.ttf', fontSize: 2.2, letterSpacing: 0.05, lineHeight: 1, 'material-toneMapped': false }
  const ref = useRef()
